<?php
	function db_connect(){
		...
		$conn = mysqli_connect("localhost", "root", "\$this->@SMR_2005", "www_project");
		if (! $conn){
	...
	function getOrderItem($conn, $orderId){
		$query = "SELECT book_isbn FROM order_items WHERE orderid = '$orderId'";
		$result = mysqli_query($conn, $query);
		if (! $result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
		...
		if (mysqli_num_rows($result) == 0){
			echo "Order doesn't exisits !";
			exit;
		}
		// fetch the book name from the book ISBN
		$row = mysqli_fetch_assoc($result);
		$book = $row['book_isbn'];
		$query2 = "SELECT * FROM books WHERE book_isbn = '$book'";
		...
		if (mysqli_num_rows($result2) == 0){
			echo "Error ! Book doesn't exisits.";
			exit;
		}
		$row2 = mysqli_fetch_assoc($result2);
		return $row2['book_title'] . " by ' " . $row2['book_author'] . " '";
	}
?>